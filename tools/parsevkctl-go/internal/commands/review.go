...
func RunTaskReview(ctx context.Context, input TaskReviewRunInput) int {
	fmt.Fprintln(input.Stdout, "WARNING: task review is deprecated. Use $parsevk-pr-review instead.")
...
func BuildTaskReview(ctx context.Context, input TaskIssueInput) (ReviewResult, error) {
	...
	result := ReviewResult{
		...
		Scope:        []string{"OK: manual scope review required"},
		Secrets:      []string{"OK: manual secret review required"},
		Blockers:     []string{},
