...
var OkUsersGetInfoService_1;
import { Injectable, Logger } from '@nestjs/common';
import { signOkRequestForUsersGetInfo, } from '../ok-friends-signature.util.js';
import { OkAuthService } from './ok-auth.service.js';
const OK_USERS_GET_INFO_FIELDS = [
    'accessible',
    'age',
    'allowed_for_ads_vk',
...
let OkUsersGetInfoService = OkUsersGetInfoService_1 = class OkUsersGetInfoService {
    auth;
    logger = new Logger(OkUsersGetInfoService_1.name);
    constructor(auth) {
        this.auth = auth;
    }
    async usersGetInfo(params) {
        ...
        if (params.uids.length > MAX_UIDS_PER_REQUEST) {
            throw new Error(`uids parameter must contain at most ${MAX_UIDS_PER_REQUEST} user IDs per request`);
        }
        ...
        if (!apiParams.fields) {
            this.logger.error('OK API users.getInfo: fields is missing before signature calculation!');
        ...
        this.logger.log(`OK API users.getInfo params for signature: ${JSON.stringify(paramsForSignature)}`);
        const sig = signOkRequestForUsersGetInfo(paramsForSignature, accessToken, applicationSecretKey);
        ...
        this.logger.log(`OK API users.getInfo params: uids count=${params.uids.length}, fields=${(apiParams.fields ?? '').substring(0, 100)}`);
        try {
            ...
            if (!response.ok) {
                ...
                this.logger.error(`OK API users.getInfo error: ${response.status} ${response.statusText} - ${errorText}`);
                throw new Error(`OK API users.getInfo request failed: ${response.status} ${response.statusText}`);
            }
            ...
            if ('users' in data && Array.isArray(data.users)) {
                this.logger.log(`OK API users.getInfo returned ${data.users.length} users`);
                return data.users;
            }
            this.logger.error(`OK API users.getInfo returned invalid response format. Response structure: ${JSON.stringify(data).substring(0, 500)}`);
            throw new Error('OK API users.getInfo returned invalid response format');
        }
        catch (error) {
            if (error instanceof Error) {
                this.logger.error(`OK API users.getInfo request failed: ${error.message}`, error.stack);
                throw error;
            }
    ...
    resolveFields(fields) {
        if (!fields || fields.length === 0) {
            this.logger.log(`OK API users.getInfo: using all available fields (${OK_USERS_GET_INFO_FIELDS.length} fields)`);
        ...
        if (invalidFields.length > 0) {
            this.logger.warn(`OK API users.getInfo: ignoring unsupported fields (${invalidFields.length}): ${invalidFields.join(', ')}`);
        }
        if (validFields.length === 0) {
            this.logger.log(`OK API users.getInfo: provided fields empty or invalid, using default list (${OK_USERS_GET_INFO_FIELDS.length} fields)`);
            return OK_USERS_GET_INFO_FIELDS.join(',');
        }
        this.logger.log(`OK API users.getInfo: using provided fields (${validFields.length} fields)`);
        return validFields.join(',');
    }
    checkApiError(data) {
        if ('error' in data && data.error) {
            const errorData = data.error;
            this.logger.error(`OK API users.getInfo error: ${errorData.error_code} - ${errorData.error_msg}`);
            throw new Error(`OK API users.getInfo error: ${errorData.error_msg ?? 'Unknown error'}`);
        }
        if ('error_code' in data && data.error_code) {
            const errorResponse = data;
            const errorMsg = errorResponse.error_msg ?? 'Unknown error';
            this.logger.error(`OK API users.getInfo error: ${errorResponse.error_code} - ${errorMsg}`);
            throw new Error(`OK API users.getInfo error: ${errorMsg}`);
        }
    }
};
OkUsersGetInfoService = OkUsersGetInfoService_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [OkAuthService])
], OkUsersGetInfoService);
export { OkUsersGetInfoService };
//# sourceMappingURL=ok-users-get-info.service.js.map