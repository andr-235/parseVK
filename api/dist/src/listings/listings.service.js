...
import { formatCsvHeader, formatCsvRow, CSV_DEFAULT_FIELDS, CSV_FIELD_LABELS, } from './utils/csv-exporter.js';
let ListingsService = ListingsService_1 = class ListingsService {
    repository;
    logger = new Logger(ListingsService_1.name);
    constructor(repository) {
        this.repository = repository;
    }
    async getListings(options) {
        const { page, pageSize, search, source, archived, sortBy, sortOrder } = options;
        const skip = (page - 1) * pageSize;
        const order = sortOrder ?? 'asc';
        const orderBy = sortBy
            ? [
                {
                    [sortBy]: sortBy === 'contactPhone'
                        ? { sort: order, nulls: 'last' }
                        : order,
                },
            ]
            : [{ createdAt: 'desc' }];
        this.logger.debug(`[getListings] sort: ${sortBy ?? 'default'}, order: ${order}`);
        const where = {};
        if (search) {
        ...
        const distinctSources = result.distinctSources;
        const items = listings.map((listing) => ListingMapper.toDto(listing));
    ...
    async getListingsForExport(options) {
    ...
    async *iterateAllListings(options) {
        const { search, source, archived } = options;
    ...
    async *exportAsCsvLines(options) {
    ...
    buildUpdateData(payload, existing) {
        ...
        const overrides = new Set(currentOverrides);
...
ListingsService = ListingsService_1 = __decorate([
    Injectable(),
