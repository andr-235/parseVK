...
export function validate(config) {
    const validatedConfig = plainToInstance(AppConfig, {
        ...
        vkApiTimeoutMs: config.VK_API_TIMEOUT_MS
            ? parseInt(config.VK_API_TIMEOUT_MS, 10)
            : 30000,
        ...
        imageModerationTimeoutMs: config.IMAGE_MODERATION_TIMEOUT_MS
            ? parseInt(config.IMAGE_MODERATION_TIMEOUT_MS, 10)
            : undefined,
        ...
        jwtAccessExpiresInMinutes: config.JWT_ACCESS_EXPIRES_IN_MINUTES
            ? parseInt(config.JWT_ACCESS_EXPIRES_IN_MINUTES, 10)
            : 15,
        jwtRefreshExpiresInDays: config.JWT_REFRESH_EXPIRES_IN_DAYS
            ? parseInt(config.JWT_REFRESH_EXPIRES_IN_DAYS, 10)
            : 7,
        authLoginRateLimitTtlSeconds: config.AUTH_LOGIN_RATE_LIMIT_TTL_SECONDS
            ? parseInt(config.AUTH_LOGIN_RATE_LIMIT_TTL_SECONDS, 10)
            : 60,
        authLoginRateLimitMaxAttempts: config.AUTH_LOGIN_RATE_LIMIT_MAX_ATTEMPTS
            ? parseInt(config.AUTH_LOGIN_RATE_LIMIT_MAX_ATTEMPTS, 10)
            : 5,
        vkApiRateLimitRequests: config.VK_API_RATE_LIMIT_REQUESTS
            ? parseInt(config.VK_API_RATE_LIMIT_REQUESTS, 10)
            : 3,
        vkApiRateLimitWindowMs: config.VK_API_RATE_LIMIT_WINDOW_MS
            ? parseInt(config.VK_API_RATE_LIMIT_WINDOW_MS, 10)
            : 1000,
        vkApiRetryMaxAttempts: config.VK_API_RETRY_MAX_ATTEMPTS
            ? parseInt(config.VK_API_RETRY_MAX_ATTEMPTS, 10)
            : 2,
        vkApiRetryInitialDelayMs: config.VK_API_RETRY_INITIAL_DELAY_MS
            ? parseInt(config.VK_API_RETRY_INITIAL_DELAY_MS, 10)
            : 500,
        vkApiRetryMaxDelayMs: config.VK_API_RETRY_MAX_DELAY_MS
            ? parseInt(config.VK_API_RETRY_MAX_DELAY_MS, 10)
            : 2000,
        vkApiRetryMultiplier: config.VK_API_RETRY_MULTIPLIER
            ? parseFloat(config.VK_API_RETRY_MULTIPLIER)
            : 2,
        vkApiCircuitBreakerFailureThreshold: config.VK_API_CIRCUIT_BREAKER_FAILURE_THRESHOLD
            ? parseInt(config.VK_API_CIRCUIT_BREAKER_FAILURE_THRESHOLD, 10)
            : 5,
        vkApiCircuitBreakerResetTimeoutMs: config.VK_API_CIRCUIT_BREAKER_RESET_TIMEOUT_MS
            ? parseInt(config.VK_API_CIRCUIT_BREAKER_RESET_TIMEOUT_MS, 10)
            : 60000,
        vkApiCircuitBreakerHalfOpenMaxCalls: config.VK_API_CIRCUIT_BREAKER_HALF_OPEN_MAX_CALLS
            ? parseInt(config.VK_API_CIRCUIT_BREAKER_HALF_OPEN_MAX_CALLS, 10)
            : 3,
        ...
        monitorMessageCreatedAtColumn: config.MONITOR_MESSAGE_CREATED_AT_COLUMN || 'created_at',
        ...
        monitorMessageMetadataColumn: config.MONITOR_MESSAGE_METADATA_COLUMN || 'metadata',
    ...
    if (errors.length > 0) {
        const errorMessages = errors
            ...
            .join('; ');
        throw new Error(`Ошибка валидации конфигурации: ${errorMessages}. Проверьте переменные окружения.`);
    }
    if (!validatedConfig.vkToken) {
        throw new Error('VK_TOKEN обязателен для работы приложения. Установите переменную окружения VK_TOKEN.');
    }
    if (!validatedConfig.jwtAccessSecret) {
        throw new Error('JWT_ACCESS_SECRET обязателен для работы приложения. Установите переменную окружения JWT_ACCESS_SECRET.');
    }
    if (!validatedConfig.jwtRefreshSecret) {
        throw new Error('JWT_REFRESH_SECRET обязателен для работы приложения. Установите переменную окружения JWT_REFRESH_SECRET.');
    }
