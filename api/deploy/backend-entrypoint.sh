...
if [ -f /proc/net/route ]; then
  ...
  cat /proc/net/route | head -5 | while read -r line; do
...
if command -v arp >/dev/null 2>&1; then
  ...
  arp -a | head -10 | while read -r line; do
    log_debug "ARP: $line"
  done
fi
...
env | grep -i "dokploy\|database\|db\|postgres" | while read -r line; do
...
if [ ! -f ./dist/src/generated/prisma/client/index.js ] && [ ! -f ./dist/generated/prisma/client/index.js ]; then
  ...
  echo "Prisma Client уже собран, пропуск generate."
fi
