...
export class OkApiService {
  ...
  async usersGetInfo(params: OkUsersGetInfoParams): Promise<OkUserInfo[]> {
    ...
    // Если fields не указан или содержит невалидные значения, используем список доступных полей
    if (params.fields && params.fields.length > 0) {
      ...
      if (validFields.length > 0) {
        apiParams.fields = validFields.join(',');
        this.logger.log(
    ...
    if (!apiParams.fields) {
      ...
      apiParams.fields = OK_USERS_GET_INFO_FIELDS.join(',');
      this.logger.log(
        `OK API users.getInfo: fields set via fallback (${OK_USERS_GET_INFO_FIELDS.length} fields)`,
      );
    }
