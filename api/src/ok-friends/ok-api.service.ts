...
export class OkApiService {
  ...
  async usersGetInfo(params: OkUsersGetInfoParams): Promise<OkUserInfo[]> {
    ...
    if (params.fields && params.fields.length > 0) {
      ...
      // Список проверенных полей из документации OK API users.getInfo
      // Удалены несуществующие поля (badge_id, badge_img, badge_link, badge_title)
      const allFields = [
        'uid',
        'first_name',
        'last_name',
    ...
    if (!apiParams.fields) {
      ...
      // Fallback: используем минимальный набор проверенных полей
      const allFields = [
        'uid',
        'first_name',
        'last_name',
