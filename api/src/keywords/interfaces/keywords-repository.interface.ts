...
export interface IKeywordsRepository {
  ...
  update(where: { id: number }, data: {
    category?: string | null;
    isPhrase?: boolean;
  }): Promise<Keyword>;
  delete(where: { id: number }): Promise<void>;
  deleteMany(): Promise<{ count: number }>;
  findManyWithSelect(select: { id: true; word: true; isPhrase: true }): Promise<Array<{
    id: number;
    word: string;
    isPhrase: boolean;
  }>>;
  ...
  findCommentsBatch(params: { skip: number; take: number }): Promise<Array<{
    id: number;
    text: string | null;
  }>>;
  findPostsBatch(params: { skip: number; take: number }): Promise<Array<{
    id: number;
    ownerId: number;
    vkPostId: number;
    text: string | null;
