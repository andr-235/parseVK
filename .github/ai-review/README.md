...
`AGENTS.md` является единственным источником репозиторных правил reviewer. Перед запуском OpenCode доверенный helper загружает применимые файлы только из `base`-коммита Pull Request:
...
Формализуемые требования не оставляются на усмотрение модели. Trusted enforcer извлекается из того же immutable base и выполняет детерминированные проверки до обработки результата OpenCode.
...
- создаёт `major` finding с `confidence=1.0`, фактическим числом строк и ссылкой на источник правила;
- привязывает finding к изменённой строке и блокирует batch;
- не создаёт выдуманный лимит, если распознаваемого правила в `AGENTS.md` нет.

Детерминированные findings используют тот же JSON-контракт, проходят тот же валидатор и объединяются с findings модели. Prompt просит модель не дублировать уже выполненную проверку размера файла.
...
Publisher, AGENTS enforcer и batch helpers дополнительно проверяются Ruff и статическими contract-тестами обоих workflow.
