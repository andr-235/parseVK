...
async def test_run_export_job_success(service: VkFriendsExportService):
    ...
    with patch.object(service, "_get_vk_client", return_value=mock_client), \
         patch("app.modules.vk_friends.exporter.write_xlsx_file", return_value="/tmp/test.xlsx") as mock_write:
...
async def test_api_routes():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        res = await ac.post("/internal/vk/friends/export", json=start_payload, headers=headers)
        ...
        # Get raw logs
        res_logs = await ac.get(f"/internal/vk/friends/jobs/{job_id}/logs/raw", headers=headers)
