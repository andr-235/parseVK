...
class CanonicalVkCommandRepository:
    ...
    async def request_cancellation(
        ...
        *,
        ...
        stmt = select(VkTaskRunBinding).where(
            VkTaskRunBinding.task_id == task_id,
            VkTaskRunBinding.status.in_(ACTIVE_BINDING_STATUSES),
        )
        if run_id is not None:
            stmt = stmt.where(VkTaskRunBinding.run_id == run_id)
        binding = await self.session.scalar(stmt.with_for_update())
        if binding is None:
            return None
    ...
    async def _attach_source(self, *, binding, command, requested):
        ...
        duplicate = await self.session.scalar(
            select(VkCollectionDemand)
            .where(VkCollectionDemand.demand_id == requested.demand_id)
            .with_for_update()
        )
        if duplicate is not None:
            ...
            return SourceDemandAttachment(
                ...
                demand=_demand_entity(duplicate),
