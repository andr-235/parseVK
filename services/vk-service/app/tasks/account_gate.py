"""In-memory negative filter for provider account status before claiming."""
...
class AccountGate:
    ...
    Cached invalid/disabled short-circuits without a DB call; cooling_down
    short-circuits until its cooldown expires; everything else falls through
    to the DB, which stays authoritative. Call ``invalidate()`` on every
    ...
    async def can_claim(self) -> bool:
        ...
        if self._status in _BLOCKED_STATUSES:
            self._log_blocked()
            return False
        if (
    ...
    async def _load_account(self):
        async with self._session_factory() as session:
            return await self._provider_accounts_factory(session).get_by_key(self._account_key)

    def _log_blocked(self) -> None:
        if not self._warned_blocked:
            logger.warning(
                "claim blocked by account gate: account=%s status=%s",
                self._account_key,
                self._status,
            )
