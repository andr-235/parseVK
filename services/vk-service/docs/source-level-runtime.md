...
The runtime has no legacy `task.created` execution bridge and no aggregate group-set collection identity. Cancellation is delivered through `vk.execution.cancel_requested` on the same Kafka topic and partition identity as the execution request.
