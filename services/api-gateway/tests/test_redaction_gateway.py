...
def test_redact_secrets_gateway():
    with patch.object(settings, "internal_service_token", "super_gateway_token_123"):
        assert (
            redact_secrets("Token is super_gateway_token_123") == "Token is <redacted>"
        )

    # Check headers
    assert (
        redact_secrets("Authorization: Bearer confidential_jwt")
    ...
    # Check URL params
    assert (
        redact_secrets("access_token=xyz&session_key=abc&sig=123")
        == "access_token=<redacted>&session_key=<redacted>&sig=<redacted>"
    )
...
async def test_download_xlsx_error_handling():
    ...
    adapter_mock.get_xlsx_bytes.side_effect = HTTPException(
    ...
    adapter_mock.get_xlsx_bytes.side_effect = FileNotFoundError(
        "/var/data/exports/my_secret_path/vk_friends_123.xlsx"
    )
