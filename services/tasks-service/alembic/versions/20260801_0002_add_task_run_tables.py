...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "run_revision >= 0",
            name="ck_task_runs_run_revision",
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_task_run_demands_payload_object",
        ),
    )
    op.create_index(
        "ix_task_run_source_demands_source",
        "task_run_source_demands",
        ["source_id"],
    )
...
def downgrade() -> None:
    op.execute(
        "DROP TRIGGER IF EXISTS trg_task_runs_immutable_snapshot ON task_runs"
    )
    op.execute("DROP FUNCTION IF EXISTS reject_task_run_snapshot_update()")
    op.drop_index(
        "ix_task_run_source_demands_source",
        table_name="task_run_source_demands",
    )
