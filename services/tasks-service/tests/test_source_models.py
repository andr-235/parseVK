...
class FakeSession:
    """Minimal AsyncSession stand-in for repository unit tests."""
...
def test_monitoring_source_unique_identity():
    for constraint in MonitoringSource.__table__.constraints:
        ...
        pytest.fail("uq_monitoring_sources_identity constraint missing")
...
def test_task_source_unique_pair():
    for constraint in TaskSource.__table__.constraints:
        ...
        pytest.fail("uq_task_sources_task_source constraint missing")
...
def test_scope_source_access_unique_pair():
    for constraint in ScopeSourceAccess.__table__.constraints:
        ...
        pytest.fail("uq_scope_source_access_scope_source constraint missing")
...
def test_access_scope_has_separate_created_by_column():
    ...
    assert AccessScope.__table__.c.id.type.length is None or True
...
async def test_grant_scope_source_creates_row():
    ...
    assert session.added and session.added[0].access_scope_id == scope_id
...
async def test_grant_scope_source_increments_ref_count():
    ...
    session = FakeSession(scalar_sequence=[existing])
    repo = ScopeRepository(session)
...
@pytest.mark.asyncio
async def test_internal_resolver_defers_canonical_callout():
    resolver = InternalVkSourceResolver()

    with pytest.raises(SourceNotFoundError):
        await resolver.resolve(SourceIdentity("vk", "community", "12345"))
