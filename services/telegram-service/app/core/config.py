...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file=".env", env_prefix="TELEGRAM_SERVICE_", extra="ignore"
    )  # noqa: E501

    app_name: str = "parseVK TelegramService Service"
    database_url: str = Field(
        default="postgresql+asyncpg://telegram_service:telegram_service@localhost:5432/telegram_service"
    )
    internal_service_token: str = Field(default="dev-internal-token")
    vpn_service_telegram_url: str | None = Field(
        default=None, validation_alias="VPN_SERVICE_TELEGRAM_URL"
    )
