...
_vk_ingestion_health: WorkerHealth = WorkerHealth()
...
async def lifespan(app: FastAPI):
    ...
    vk_ingestion_consumer = VkIngestionConsumer()
    ...
    async def run_ingestion():
        await vk_ingestion_consumer.run_forever()
    ...
    try:
        ...
        for task in (vk_task, ingestion_task, im_task, outbox_task):
            if task:
                task.cancel()
        for task in (vk_task, ingestion_task, im_task, outbox_task):
            if task:
        ...
        await vk_ingestion_consumer.stop()
