...
from app.infrastructure.db.mappers.authors import (
    author_to_dict,
    get_author_order,
    map_profile_to_author_row,
)
from app.infrastructure.db.models import ContentAuthor, ContentComment
...
class AuthorRepository(BaseContentRepository):
    ...
    async def _update_author_verified_at(self, vk_author_id: int) -> bool:
        return await self.update_verified_at(vk_author_id)
    ...
    async def bulk_update_author_profiles(self, profiles: list[dict]) -> int:
        return await self.bulk_update_profiles(profiles)
