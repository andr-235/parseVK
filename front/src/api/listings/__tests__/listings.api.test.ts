...
describe('listingsService.fetchListings', () => {
  ...
  it('uses gateway URL for listings fetches', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ items: [], total: 0, page: 1, pageSize: 20, hasMore: false, sources: [] }),
        { status: 200 }
      )
    ) as typeof fetch
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith('/api/v1/listings?page=1&pageSize=20', expect.any(Object))
  })
  ...
  it('uses gateway URL for create imports', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ processed: 1, created: 1, updated: 0, skipped: 0, failed: 0, errors: [] }),
        { status: 200 }
      )
    ) as typeof fetch
  ...
  it('uses gateway URLs for update delete and export', async () => {
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 7, url: 'https://example.test/flat', images: [], manualOverrides: [] }), { status: 200 }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }))
      .mockResolvedValueOnce(new Response(new Blob(['ID\n'], { type: 'text/csv' }), { status: 200, headers: { 'Content-Disposition': 'attachment; filename="listings.csv"' } })) as typeof fetch
    ...
    expect(globalThis.fetch).toHaveBeenNthCalledWith(1, '/api/v1/listings/7', expect.objectContaining({ method: 'PATCH' }))
    expect(globalThis.fetch).toHaveBeenNthCalledWith(2, '/api/v1/listings/7', expect.objectContaining({ method: 'DELETE' }))
    expect(globalThis.fetch).toHaveBeenNthCalledWith(3, '/api/v1/listings/export?fields=id', expect.any(Object))
