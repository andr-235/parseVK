...
export const keywordsService = {
  async addKeyword(
    ...
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/add`, {
        method: 'POST',
  ...
  async updateKeywordCategory(id: number, category?: string | null): Promise<IKeywordResponse> {
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/${id}`, {
        method: 'PATCH',
  ...
  async bulkAddKeywords(words: string[]): Promise<IBulkAddResponse> {
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/bulk-add`, {
        method: 'POST',
  ...
  async uploadKeywords(file: File): Promise<IBulkAddResponse> {
    try {
      ...
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/upload`, {
        method: 'POST',
        body: formData,
      })
  ...
  async deleteAllKeywords(): Promise<IDeleteResponse> {
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/all`, {
        method: 'DELETE',
      })
  ...
  async recalculateKeywordMatches(): Promise<{
    ...
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/recalculate-matches`, {
        method: 'POST',
      })
  ...
  async rebuildKeywordForms(): Promise<IKeywordFormsRebuildResponse> {
    try {
      const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/rebuild-forms`, {
        method: 'POST',
      })
  ...
  async getKeywordForms(id: number): Promise<IKeywordFormsResponse> {
    const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/${id}/forms`)
  ...
  async addManualKeywordForm(id: number, form: string): Promise<IKeywordFormsResponse> {
    const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/${id}/forms/manual`, {
      method: 'POST',
  ...
  async addKeywordFormExclusion(id: number, form: string): Promise<IKeywordFormsResponse> {
    const response = await createRequest(`${GATEWAY_API_URL}/v1/keywords/${id}/forms/exclusions`, {
      method: 'POST',
      body: JSON.stringify({ form }),
    })
