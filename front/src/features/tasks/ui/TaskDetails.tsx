...
import { getTaskStatusText, getGroupStatusText } from '@/features/tasks/lib/statusHelpers'
import { calculateTaskProgress } from '@/features/tasks/lib/taskProgress'
import ProgressBar from '@/components/ProgressBar'
import { Button } from '@/components/ui/button'
import { useTaskDetails } from '@/features/tasks/model/useTaskDetails'
...
function TaskDetails({ task, onClose }: TaskDetailsProps) {
  ...
  const pendingGroups =
    groupStatusDistribution.pending > 0 ? groupStatusDistribution.pending : pendingGroupsCalculated
    ...
    if (group.status === 'processing' || group.status === 'running') {
      ...
      if (progressInfo.total != null && progressInfo.total > 0) {
        if (progressInfo.processed != null) {
          ...
          barCurrent = clamp(
            (progressInfo.percent / 100) * progressInfo.total,
            0,
            progressInfo.total
          )
        }
      ...
      return renderProgress(
        'primary',
        hasConcreteProgress ? barCurrent : 0,
        hasConcreteProgress ? barTotal : 1,
        percentText,
        <span className="text-muted-foreground">Парсинг...</span>,
        !hasConcreteProgress
      )
    }
          ...
          {totalGroups > 0 && (
            <div className="rounded-2xl border border-border/50 bg-background-secondary/70 p-6">
              ...
              <ProgressBar
                ...
                className="h-2 mb-4"
              />
              <div className="text-xs font-medium text-muted-foreground mb-6">
                  ...
                  {task.groups.length === 0 ? (
                    ...
                    task.groups.map((group, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 flex justify-end">{renderGroupResult(group)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
