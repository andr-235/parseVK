...
import { usePhotoAnalysisStore } from '@/pages/author-analysis/store'
import { calculateAnalysisParams } from '@/pages/author-analysis/utils/authorAnalysisUtils'
import type { AnalyzePhotosOptions } from '@/pages/author-analysis/types/photoAnalysis'
...
export const usePhotoAnalysis = (
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Не удалось загрузить результаты анализа'
        toast.error(message)
      }
    }
  ...
  const handleAnalyze = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Не удалось проанализировать фотографии'
        toast.error(message)
      }
    },
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Не удалось удалить результаты анализа'
      toast.error(message)
    }
  }, [isValidAuthor, deleteResults, vkUserId])

  const handleFilterChange = useCallback(
    (nextFilter: 'all' | 'suspicious') => {
