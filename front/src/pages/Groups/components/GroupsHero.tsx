  ...
  const steps = [
    {
      ...
      badgeClassName:
        'flex h-11 w-11 items-center justify-center rounded-full bg-[rgba(52,152,219,0.18)] text-base font-semibold text-[#1b4f72] md:h-12 md:w-12 md:text-lg dark:bg-[rgba(93,173,226,0.32)] dark:text-[rgba(236,240,241,0.9)]',
      titleClassName: 'mb-1 text-base font-semibold text-text-primary',
      descriptionClassName: 'text-sm leading-relaxed text-text-secondary',
      title: 'Добавьте ссылку',
      description: 'Вставьте URL сообщества ВК и нажмите «Добавить».',
    },
    {
      itemClassName: 'flex items-start gap-4',
      badgeClassName:
      ...
      description: 'Загрузите текстовый список ссылок, чтобы ускорить наполнение.',
    },
    {
      itemClassName: 'flex items-start gap-4',
      badgeClassName:
      ...
      description: 'Отслеживайте прогресс — новые записи сразу появятся в таблице.',
    },
  ]
        ...
        <div className="flex flex-col gap-5 rounded-[24px] border border-border bg-background-secondary p-6 shadow-[0_22px_44px_-36px_rgba(0,0,0,0.3)] md:gap-6 md:p-7">
          ...
          <ol className="space-y-4 md:space-y-5">
            {steps.map((step, index) => (
              <li key={step.title} className={step.itemClassName}>
                <span className={step.badgeClassName}>{index + 1}</span>
                <div>
