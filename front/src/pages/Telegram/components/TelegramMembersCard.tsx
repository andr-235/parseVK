            ...
            <TableBody>
              {members.length === 0 ? (
                 <TableRow>
                  ...
                  members.map((member) => {
                    const flags = []
                    if (member.verified) flags.push({ icon: <CheckCircle2 className="h-3 w-3" />, title: 'Верифицирован', variant: 'default' as const })
                    if (member.scam) flags.push({ icon: <Shield className="h-3 w-3" />, title: 'Scam', variant: 'destructive' as const })
                    if (member.fake) flags.push({ icon: <UserX className="h-3 w-3" />, title: 'Fake', variant: 'outline' as const })
                    if (member.deleted) flags.push({ icon: <Ban className="h-3 w-3" />, title: 'Удален', variant: 'secondary' as const })
                    if (member.isPremium) flags.push({ icon: <Star className="h-3 w-3" />, title: 'Premium', variant: 'default' as const })
                      ...
                      <TableRow key={`${member.userId}-${member.telegramId}`}>
                        <TableCell>
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">ID: {member.telegramId}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex flex-col text-sm">
                            {member.username ? (
                                <span className="text-primary hover:underline cursor-pointer">@{member.username}</span>
                            ) : (
                                <span className="text-muted-foreground">—</span>
                            )}
                            {member.phoneNumber && <span className="text-xs text-muted-foreground">{member.phoneNumber}</span>}
                          </div>
                        </TableCell>
                        <TableCell>
                           <div className="flex flex-wrap gap-1">
                            {flags.map((flag, i) => (
                                <Badge key={i} variant={flag.variant} className="h-5 w-5 p-0 flex items-center justify-center rounded-full" title={flag.title}>
                                    {flag.icon}
                                </Badge>
                            ))}
                             {flags.length === 0 && <span className="text-muted-foreground text-xs">—</span>}
                           </div>
                        </TableCell>
                        <TableCell>
