...
import type { ExportJobStatus, JobLogLevel } from '@/types/okFriendsExport/okFriendsExport'
import {
  STATUS_LABELS,
  STATUS_VARIANTS,
  toOptionalNumber,
  toOptionalString,
} from '@/utils/okFriendsExport/okFriendsExportUtils'
...
const normalizeLogs = (logs: OkFriendsJobLog[]): JobLogEntry[] => {
...
export const useOkFriendsExport = () => {
    ...
    (event: OkFriendsStreamEvent) => {
      ...
      if (event.type === 'done') {
        ...
        if (event.data.xlsxPath && event.data.jobId) {
          void okFriendsExportService.downloadJobFile(event.data.jobId, 'xlsx').catch(() => {})
        }
        return
      }
  ...
  const connectStream = useCallback(
    (id: string) => {
      closeStream()
      const { close } = okFriendsExportService.streamJob(id, {
  ...
  const loadJob = useCallback(async (id: string) => {
    try {
      const response = await okFriendsExportService.getJob(id)
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const response = await okFriendsExportService.export({
        params,
      })
  ...
  const handleDownloadXlsx = useCallback(async () => {
    if (!jobId) return
    try {
      await okFriendsExportService.downloadJobFile(jobId, 'xlsx')
    } catch {
