...
| Метод | Путь | Upstream | Описание |
|-------|------|----------|----------|
| GET | `/api/v1/im/search/messages` | content-service (configurable) | Поиск сообщений мессенджеров (GET) |
| POST | `/api/v1/im/messages/search` | content-service (configurable) | Поиск сообщений мессенджеров (POST) |
| GET | `/api/v1/im/notifier/state` | im-service | Состояние нотификатора |
| PUT | `/api/v1/im/notifier/state` | im-service | Обновить состояние нотификатора |
| GET | `/api/v1/im/notifier/new-messages` | im-service | Новые сообщения нотификатора |
| GET | `/api/v1/im/groups` | im-service | Список IM-групп |
| POST | `/api/v1/im/groups` | im-service | Создать IM-группу |
| PATCH | `/api/v1/im/groups/{id}` | im-service | Обновить IM-группу |
| DELETE | `/api/v1/im/groups/{id}` | im-service | Удалить IM-группу |

Search endpoints accept the following parameters:
- `messenger` — фильтр по мессенджеру
- `q` / `query` — текст поиска (ILIKE)
- `chatId` / `chat_id` — фильтр по чату
- `author` — фильтр по автору
- `page` / `limit` — пагинация
- `dateFrom` / `dateTo` — фильтр по дате
- `onlyWithKeywords` + `keywords` — режим поиска по ключевым словам (Python batch scan)

Ответ режима keyword search содержит поле `scanned` — количество просканированных строк (полезно для мониторинга производительности).

Search backend может быть переключен через переменную окружения `GATEWAY_IM_SEARCH_BACKEND=content|im` (по умолчанию content).
