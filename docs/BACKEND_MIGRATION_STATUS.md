...
| `comments/comments.controller.ts` | `comments` | `api-gateway /api/v1/comments/*` -> `moderation-service /internal/moderation/*` | `migrated` | Frontend comments flow is migrated to FastAPI/gateway. `GET /comments`, `GET /comments/cursor`, `PATCH /comments/:id/read`, and `POST /comments/search` are all served by the gateway, backed by `moderation-service`. Advanced Elasticsearch parity is intentionally replaced by moderation-service fallback search for now. `keywordSource` filter is a compatibility no-op until storage model contains source info. |
...
| `data-import/data-import.controller.ts` | `data` | `api-gateway /api/v1/data/import` -> `content-service /internal/content/data/import` | `migrated` | Listings import is migrated to gateway/content-service with JSON and multipart JSON upload support. Legacy NestJS fallback code is retained until a cleanup issue. Existing production data backfill from the old Prisma DB is deferred unless handled by a separate rollout. |
...
| `listings/listings.controller.ts` | `listings` | `api-gateway /api/v1/listings/*` -> `content-service /internal/content/listings/*` | `migrated` | Listings list, update, delete, and CSV export are migrated to gateway/content-service. Legacy NestJS fallback code is retained until a cleanup issue. New imports write to `content-db`; production backfill from legacy Prisma storage is deferred. |
